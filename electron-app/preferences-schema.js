...
const PREFERENCES = {
  ...
  ackVolume: {
    ...
    max: 1,
    description:
      'Playback gain for SHORT acknowledgement phrases ("mm-hmm", "right") that ' +
      'land WHILE someone is still speaking, relative to normal speech at 1. ' +
      'Acks used to play at exactly the volume of a real reply, which is what ' +
      'made them feel like the bot butting in rather than backchanneling — a ' +
      'person murmuring agreement is markedly quieter than a person taking the ' +
      'floor. Two acks are NOT affected and always play at full volume. LONG ' +
      'acks ("Let me think about that.") are a different speech act, announcing ' +
      'that the floor has changed hands, and something taking the floor at a ' +
      'third of normal volume is just hard to hear. And any ack landing in ' +
      'SILENCE is not backchannel at all, whatever pool it came from: with ' +
      'nobody to murmur under, the bot is taking the floor and wants to be ' +
      'heard. Every other utterance plays at full gain. Lip-sync is driven from the pre-gain signal, so a quiet ' +
