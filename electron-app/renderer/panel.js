...
sessionPerCalendarInviteesInput?.addEventListener('change', () => {
  // Through setConfig, not a bare invoke (#557): a silently-failed write here is
  // the worst one in the pane — the checkbox would show per-meeting sessions ON
  // while every call kept landing in the shared directory, which is exactly the
  // isolation this setting exists to promise.
  setConfig('sessionPerCalendarInvitees', sessionPerCalendarInviteesInput.checked);
