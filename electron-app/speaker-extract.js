// speaker-extract.js — per-PERSON audio out of Meet's shuffled slot tracks
// (#209/#422).
//
...
// whole-call majority vote — is wrong wherever a slot changed hands.
//
// WHAT MAKES IT SOLVABLE. Reassignment happens across SILENCE, never mid-word
// (confirmed by ear in Audacity, 2026-08-18). So a contiguous burst of energy on
// one track is one person for its whole length, and the job is labelling a few
// hundred segments rather than continuously re-attributing audio.
//
...
//
// Neither is the detector verdict, and that is the point. Nothing here depends
// on WINDOW_MS, ARM, RELEASE or METER_HOLD_MS, so the output can serve as ground
// truth for tuning those constants without circularity. Only the indicator poll
// cadence bounds resolution, and a sampling rate biases no detector.
//
// OVERLAP IS RESOLVED BY EXCLUSION, NOT BY GUESSING. Two people talking at once
// are necessarily in different slots, so the window is an assignment problem.
// Where lit names outnumber sounding slots it stays `unknown` and is emitted as
// an excerpt — an indicator lit with nothing behind it and no spare slot to
...
function wavHeader(dataBytes, { sampleRate = 48000, channels = 1, bits = 16 } = {}) {
...
function frameDb(samples, frameLen) {
...
function segmentsFromDb(db, { frameMs = 20, gapMs = 300, minMs = 200,
...
function litIntervals(events, { holdMs = 260, exclude = [] } = {}) {
  const by = new Map();
...
const overlapMs = (a, b, c, d) => Math.max(0, Math.min(b, d) - Math.max(a, c));

function overlapWith(intervals, a, b) {
  let acc = 0;
...
function bestAssignment(scoreMatrix, { bonus = null } = {}) {
...
function candidatesFor(seg, lit, { minShare = 0.30 } = {}) {
...
// one at -12.5dB.
function weakCandidatesFor(seg, lit, { minShare = 0.30 } = {}) {
  const dur = seg.endMs - seg.startMs;
  const out = [];
  for (const [name, iv] of lit) {
    const o = overlapWith(iv, seg.startMs, seg.endMs);
...
function attribute(trackSegs, lit, { dominance = 1.8, continuityBonus = 0.25 } = {}) {
  const all = [];
...
// The binary comes from the caller so there is still exactly one place that
// knows how to find it (call-media-merge.js resolveFfmpegPath) — a packaged app
// ships its own and cannot assume one on PATH.
const runFfmpeg = (bin, args) =>
  execFileSync(bin, ['-nostdin', '-loglevel', 'error', '-y', ...args]);

function decodeToWav(bin, src, dst, rate) {
  runFfmpeg(bin, ['-i', src, '-ac', '1', '-ar', String(rate), '-c:a', 'pcm_s16le', dst]);
  return dst;
}
...
// `writeAudio` is the expensive half, and separable on purpose: the labels and
// attribution.json are a few hundred KB and answer every analysis question,
// while the per-person WAVs are ~300 MB PER PERSON for an hour-long call. Making
// them unconditional would mean a gigabyte per call for output most calls never
// get opened. The decode cache under .wav/ is removed with `cleanup` for the
// same reason: it is another ~300 MB per slot and rebuildable in seconds.
async function extractSpeakerTracks({
  tracksDir, eventsFile, outDir, rate = 48000, ffmpegPath = 'ffmpeg',
  writeAudio = true, cleanup = false, log = console.log,
} = {}) {
  if (!tracksDir || !eventsFile) throw new Error('tracksDir and eventsFile are required');
  if (!existsSync(eventsFile)) {
    // Identity comes from the indicator capture; without it there is nothing to
    // attribute WITH, and guessing from the manifest names is the bug this
    // whole module exists to undo. Say so rather than emitting a plausible file.
    return { ok: false, reason: 'no speaking-events.jsonl (speakingEventCapture off?)' };
  }
  if (!existsSync(path.join(tracksDir, 'manifest.json'))) {
    return { ok: false, reason: 'no manifest.json in tracks dir' };
  }
  outDir = outDir || path.join(tracksDir, 'by-speaker');
  const console_ = { log };
  ...
  log(`call ${manifest.callId} — ${audio.length} remote slots, bot "${botName}"`);
  ...
  for (const t of audio) {
    ...
    if (!existsSync(wav)) decodeToWav(ffmpegPath, path.join(tracksDir, t.file), wav, rate);
    ...
    log(`  ${t.track.padEnd(22)} ${String(segments.length).padStart(4)} segments  `
  ...
  for (const k of Object.keys(stat).sort((a, b) => stat[b] - stat[a])) {
    log(`  ${k.padEnd(18)} ${String(stat[k]).padStart(4)} segments  ${(100 * time[k] / voiced).toFixed(1)}% of voiced time`);
  }
  ...
  for (const [name, list] of writeAudio ? byPerson : []) {
    ...
    log(`  ${path.basename(file).padEnd(24)} ${String(list.length).padStart(4)} segments  ${mins.toFixed(1)} min voiced`);
  }
  ...
  writeFileSync(path.join(outDir, 'attribution.json'), JSON.stringify({
    callId: manifest.callId, t0: T0, generated: 'speaker-extract.js',
  ...
  if (unresolved.length) {
    ...
    for (const [i, s] of unresolved.entries()) {
      ...
      runFfmpeg(ffmpegPath, ['-ss', String(from), '-t', String(dur), '-i', path.join(tracksDir, src.file),
  ...
  if (cleanup) { try { rmSync(workDir, { recursive: true, force: true }); } catch { /* best-effort */ } }
  log(`\ndone -> ${outDir}`);
  return { ok: true, outDir, segments: segs.length, stats: stat, timeByMethod: time,
           people: [...byPerson.keys()], unresolved: unresolved.length };
}


module.exports = {
  extractSpeakerTracks,
  // exported for tests — the attribution decisions are where this can be
  // confidently wrong, and they are testable without a byte of audio
  parseWavHeader, wavHeader, frameDb, segmentsFromDb, litIntervals, isBotName,
  overlapMs, overlapWith, bestAssignment, candidatesFor, weakCandidatesFor, attribute,
};
