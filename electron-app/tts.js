...
// #383: pronunciation fixes for words every engine mangles — "vibeconferencing"
// came out as "vibey-conferencing"; a hyphen makes the engines break the
// compound correctly. A table (not a one-off) so future awkward names just add
// a row. Applied at the synthesize()/sayFallback() entry points ONLY — the
// substitution must never leak into what main.js records as the bot's words
// (transcripts, the #239 replay stash, #360 barge-in chunk texts all keep the
...
function applyTtsPronunciationFixes(text) {
  if (!text) return text;
  let out = String(text);
  for (const [pattern, replacement] of TTS_PRONUNCIATION_FIXES) {
...
class TTSProvider {
  constructor(config = {}) {
    ...
    // affects HOW it's rendered (provider/voice/model). The ack phrase pool
    // (preferences-schema.js) is small and fixed, so its handful of strings
    // land here and get reused instead of re-hitting ElevenLabs or paying
    // Windows SAPI's ~0.5-1s PowerShell cold start on every repeat. Ordinary
    // (non-repeating) reply text just never hits again and ages out — no
    // separate "is this an ack" flag needed, and no invalidation logic either:
    // the key already encodes every setting that could make the same text
    // sound different, so a changed voice/provider/model is automatically a
    // cache miss rather than stale audio.
    this._cache = new Map(); // key -> ArrayBuffer
    this._cacheCap = 32;
    this._cacheMaxWords = 10; // acks are a handful of words; real replies aren't
  }

  // Real reply text is essentially never repeated verbatim, so caching it would
  // just spend cache slots on one-off entries that evict actual ack phrases
  // before they can be reused. A word-count cutoff is a cheap proxy for "this
  // is probably a bounded ack/filler phrase, not generated prose".
  _isCacheable(text) {
    return text.trim().split(/\s+/).length <= this._cacheMaxWords;
  }
  ...
  async synthesize(text) {
    ...
    // #383: before the cache key is computed, so a prewarmed ack phrase and the
    // same phrase spoken later hash to the same entry.
    text = applyTtsPronunciationFixes(text);

    const cacheable = this._isCacheable(text);
    // A cache hit skips the queue entirely — no reason to wait behind an
    // in-flight synthesis for audio we already have.
    const key = cacheable ? this._cacheKey(text) : null;
    if (key && this._cache.has(key)) {
      const buf = this._cache.get(key);
      this._cache.delete(key);
      this._cache.set(key, buf); // refresh recency
      return buf;
    }

    // Serialize TTS requests to avoid concurrent API limit
    return new Promise((resolve, reject) => {
      this._queue.push({ text, key, resolve, reject });
      this._processQueue();
    });
  }
  ...
  _processQueue() {
    ...
    this._active = true;
    const { text, key, resolve, reject } = this._queue.shift();

    this._doSynthesize(text)
      .then((buf) => {
        if (buf && key) this._cachePut(key, buf);
        resolve(buf);
      })
      .catch(reject)
      .finally(() => {
  ...
  _cachePut(key, buf) {
    ...
    if (this._cache.size > this._cacheCap) {
      this._cache.delete(this._cache.keys().next().value); // evict least-recently-used
    }
  }
  ...
  async sayFallback(text) {
    ...
    return this._systemSay(applyTtsPronunciationFixes(text));
  }
