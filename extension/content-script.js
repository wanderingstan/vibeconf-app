...
// ---------------------------------------------------------------------------
// EARLY PATCH — Must run before ANY other code on the page.
// Injects a synchronous inline <script> that patches getUserMedia,
// enumerateDevices, and permissions.query before Meet's code can
// cache references to the originals. This fixes the signed-in
...
(function() {
  ...
  MediaDevices.prototype.getUserMedia = function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      console.log('[bots-in-calls] Early: getUserMedia queued (' + window.__botsInCallsPendingGUM.length + ')');
    });
  };
  ...
  console.log('[bots-in-calls] Early: all media APIs patched');
})();
`;
  (document.documentElement || document).prepend(patchScript);
  patchScript.remove();
  console.log('[bots-in-calls] Early patch injected from content script');
} catch (e) {
  console.error('[bots-in-calls] Early patch injection failed:', e);
}
