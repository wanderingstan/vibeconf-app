...
(function() {
  ...
  MediaDevices.prototype.getUserMedia = function() {
    // If the full handler is loaded (from page-inject.js), use it
    if (window.__botsInCallsGetUserMedia) {
      return window.__botsInCallsGetUserMedia.apply(this, arguments);
    }
    // Otherwise queue the request until the handler is ready
  ...
  // Patch getDisplayMedia similarly
  ...
  console.log('[bots-in-calls] Early patch: getUserMedia intercepted before page scripts');
})();
`;
