...
- **The whiteboard/room has a shareable URL.** If someone asks for the whiteboard link (or where to view the room in a browser), call `get_room_info` — it reports `Whiteboard / room URL: …`. Post that with `send_chat` or read it aloud.
