...
or when nobody is named at all. That gate cannot see a back-and-forth between
two other people, because such an exchange often names nobody: "what do you
think?" / "I'd push it to Friday" / "works for me". You can see it, in the
transcript.

When that is happening and nothing the bot says would help, `hold_voice` for
roughly the length of the exchange. It expires by itself, so you cannot
accidentally mute the bot for the rest of the call, and you never need to
release it. If the conversation turns back to the bot sooner, `hold_voice` with
seconds: 0.

Do not use it to make the bot generally quieter. That is what
realtimeRespondWhenUnnamed is for.
