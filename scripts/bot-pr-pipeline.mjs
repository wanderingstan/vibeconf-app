...
function workerPrompt(issue) {
  ...
  return [
    ...
    `3. NAME YOUR BRANCH FIRST, before your first commit. You are in a dedicated`,
    `   git worktree of ${short}, and it starts on an auto-generated branch called`,
    `   something like "worktree-issue-...". That name says nothing about the work.`,
    `   Run: \`git switch -c <type>/${issue.number}-<slug>\``,
    `   where <type> is fix, feat, refactor, docs, test, perf or chore — whichever`,
    `   matches the change — and <slug> is three to five kebab-case words naming`,
    `   what you did, not what the issue is called.`,
    `   e.g. fix/${issue.number}-drop-troubleshooting-agent-tail`,
    `4. Make the change and run the tests that cover it. Commit here.`,
    `5. Open a DRAFT pull request that quotes the issue's own acceptance criteria and`,
    `   says "Closes #${issue.number}": \`gh pr create --draft --fill-first\`.`,
    `6. Comment the outcome on the issue either way, including "skipped, and why".`,
    ``,
