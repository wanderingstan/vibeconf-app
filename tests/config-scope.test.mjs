...
test('APP_LEVEL_KEYS is exactly the decided set (guard against accidental promotion)', () => {
  assert.deepEqual(
    ...
    // ttsApiKeySource / ttsGiftStatus (#273): provenance and answer-state for a
    // gifted ElevenLabs key. App-level like ttsApiKey itself, but NOT in
    // MIGRATE_KEYS — a gift is tied to the logged-in account, not the machine,
    // and is cleared on logout instead (clearGiftedTtsKey in main.js).
    ['agentBackend', 'agentHosting', 'automationProbed', 'claudeIntegrationRemoved', 'confirmQuit', 'dangerousMode', 'remoteLogging', 'syncBaseUrl', 'ttsApiKey', 'ttsApiKeySource', 'ttsGiftStatus', 'updateChannel', 'vcSessionLoggedOutToken', 'vcSessionToken', 'websiteUrl'],
  );
});
